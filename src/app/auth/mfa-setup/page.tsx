...
function MFASetupContent() {
  ...
  async function initMfa() {
    try {
      ...
      const pendingToken = sessionStorage.getItem("_mfa_pending_token") || "";
      ...
      const res = await fetchInternal("/api/auth/mfa-confirm", {
        ...
        body: JSON.stringify(
          pendingToken
            ? { action: "init", mfa_pending_token: pendingToken }
            : { email, password, action: "init" },
        ),
      });
      ...
      if (data.phase === "verify") {
        ...
        setMfaPhase("verify");
        mfaTokenRef.current = data.mfa_pending_token || pendingToken;
        ...
        const msg = data.message || data.detail || data.error || "Failed to initialize MFA setup.";
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.access_token) {
        ...
        const roleFromRedirect = redirect.startsWith("/enterprise/reviewer") ? "reviewer" : "enterprise";
        const finalRole = roleFromSession || roleFromRedirect;
