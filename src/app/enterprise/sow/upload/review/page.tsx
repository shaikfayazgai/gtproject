...
import {
  ArrowRight, ArrowLeft, CheckCircle2, Edit3, RotateCcw, X,
  Check, Clock, AlertCircle, FileText, Layers,
    ...
    <motion.div variants={stagger} initial="hidden" animate="show" className="flex flex-col h-full min-h-0">
                ...
                <div key={cat.key} className="border-b border-gray-100 last:border-b-0">
                  ...
                  <div className={cn("flex items-center gap-3.5 px-5 py-4 transition-colors", isOpen ? "bg-gray-50/70" : "hover:bg-gray-50/50")}>
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      ...
                      <button onClick={() => setExpandedCat(isOpen ? null : cat.key)} className="text-[11px] font-medium text-gray-500 px-3 py-1.5 rounded-lg border border-gray-200 bg-white hover:bg-gray-50 transition-all">
                        {isOpen ? "Close" : "Edit"}
                      </button>
                    </div>
                  </div>
                            ...
                            return (
                              <div key={item.id} className={cn("px-5 py-3 border-b border-gray-100 last:border-b-0 transition-colors cursor-pointer", item.reviewState === "excluded" ? "bg-red-50/30" : "hover:bg-white/80")} onClick={() => showSource(item)}>
                                <div className="flex items-center gap-2 mb-1">
                                  ...
                                  <span className="text-[9px] font-mono text-gray-300 ml-auto">p.{item.sourcePageNumber} &middot; {item.confidence}%</span>
                                </div>
                                {isEditing ? (
                                  <div onClick={(e) => e.stopPropagation()}>
                                    <textarea autoFocus value={editDraft} onChange={(e) => setEditDraft(e.target.value)} rows={3} className="w-full text-[12px] text-gray-700 leading-relaxed px-3 py-2 rounded-lg border border-amber-300 bg-white outline-none focus:ring-2 focus:ring-amber-200 resize-none" />
                                    <div className="flex items-center gap-2 mt-1.5">
                                      <button onClick={() => saveEdit(item.id)} className="flex items-center gap-1 text-[10px] font-semibold text-white bg-amber-500 hover:bg-amber-600 px-2.5 py-1 rounded-md transition-all"><Check className="w-3 h-3" /> Save</button>
                                ...
                                {!isEditing && (
                                  <div className="flex items-center gap-1 mt-1.5 -ml-1" onClick={(e) => e.stopPropagation()}>
                                    {item.reviewState === "pending" && (<>
                                      ...
                                      <button onClick={() => startEdit(item)} className="flex items-center gap-1 text-[10px] font-medium text-amber-600 hover:bg-amber-50 px-2 py-0.5 rounded-md transition-all"><Edit3 className="w-3 h-3" /> Edit</button>
                                      <button onClick={() => updateItem(item.id, "excluded")} className="flex items-center gap-1 text-[10px] font-medium text-red-400 hover:bg-red-50 px-2 py-0.5 rounded-md transition-all"><X className="w-3 h-3" /> Exclude</button>
                                    </>)}
                                    {(item.reviewState === "accepted" || item.reviewState === "edited") && (<>
                                      <button onClick={() => startEdit(item)} className="flex items-center gap-1 text-[10px] font-medium text-amber-600 hover:bg-amber-50 px-2 py-0.5 rounded-md transition-all"><Edit3 className="w-3 h-3" /> Edit</button>
                                      <button onClick={() => updateItem(item.id, "pending")} className="flex items-center gap-1 text-[10px] font-medium text-gray-400 hover:bg-gray-100 px-2 py-0.5 rounded-md transition-all"><RotateCcw className="w-3 h-3" /> Reset</button>
                                    </>)}
                                    {item.reviewState === "excluded" && (
                                      <button onClick={() => updateItem(item.id, "pending")} className="flex items-center gap-1 text-[10px] font-medium text-gray-500 hover:bg-gray-100 px-2 py-0.5 rounded-md transition-all"><RotateCcw className="w-3 h-3" /> Restore</button>
                                    )}
                                  </div>
                                )}
                              </div>
      ...
      <motion.div variants={fadeUp} className="flex items-center justify-between mt-5">
        <button
          ...
          className="flex items-center gap-1.5 text-[12px] font-medium text-white bg-gradient-to-r from-gray-400 to-gray-500 hover:from-gray-500 hover:to-gray-600 px-4 py-2.5 rounded-xl transition-all"
        >
