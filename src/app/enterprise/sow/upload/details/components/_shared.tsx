...
import { CheckCircle2, ArrowLeft, Info, Sparkles, Loader2, ChevronDown, FileText } from "lucide-react";
import { cn } from "@/lib/utils/cn";

/* ── Section header bar ── */
export function SectionHeader({
  number, title, onAIFill, aiLoading,
}: {
  number: number;
  title: string;
  onAIFill?: () => void;
    ...
    <div className="px-6 py-5 border-b border-gray-100 flex items-center gap-3">
      ...
      {onAIFill && (
        <button
          ...
          onClick={onAIFill}
          disabled={aiLoading}
          className="ml-auto flex items-center gap-1.5 text-[11px] font-semibold text-forest-700 bg-forest-50 hover:bg-forest-100 border border-forest-200 px-3 py-1.5 rounded-xl transition-all disabled:opacity-60 shrink-0"
        >
          {aiLoading
    ...
    <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/30 flex items-center justify-between">
      ...
      {onComplete && (
        <button
          onClick={onComplete}
          className={cn(
            "flex items-center gap-2 text-[12px] font-semibold px-5 py-2.5 rounded-xl transition-all shadow-sm",
            variant === "generate"
