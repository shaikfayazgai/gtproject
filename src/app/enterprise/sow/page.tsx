...
const statusConfig: Record<string, { label: string; color: string; bg: string; icon: React.ElementType }> = {
  draft:             { label: "Draft",        color: "#706B66", bg: "rgba(0,0,0,0.05)",           icon: FileText },
  parsing:           { label: "Parsing",      color: "#3A6368", bg: "rgba(91,155,162,0.10)",      icon: Bot },
  review:            { label: "In Review",    color: "#3A6368", bg: "rgba(91,155,162,0.08)",      icon: Eye },
  approval:          { label: "In Approval",  color: "#86713D", bg: "rgba(208,176,96,0.10)",     icon: ClipboardCheck },
  approved:          { label: "Approved",     color: "#3F4735", bg: "rgba(77,87,65,0.08)",       icon: CheckCircle2 },
  rejected:          { label: "Rejected",     color: "#8B2C2C", bg: "rgba(196,87,74,0.08)",      icon: AlertTriangle },
  changes_requested: { label: "Changes Req.", color: "#86713D", bg: "rgba(208,176,96,0.08)",     icon: AlertTriangle },
  archived:          { label: "Archived",     color: "#706B66", bg: "rgba(0,0,0,0.04)",           icon: FileText },
};

const sensitivityConfig: Record<string, { label: string; color: string; bg: string }> = {
  public:       { label: "Public",       color: "#3A6368", bg: "rgba(91,155,162,0.08)" },
  internal:     { label: "Internal",     color: "#706B66", bg: "rgba(0,0,0,0.04)" },
  confidential: { label: "Confidential", color: "#86713D", bg: "rgba(208,176,96,0.08)" },
  restricted:   { label: "Restricted",   color: "#8B2C2C", bg: "rgba(196,87,74,0.06)" },
};

function riskColor(score: number): string {
  if (score <= 25) return "#3F4735";
  if (score <= 50) return "#86713D";
  return "#8B2C2C";
}
function riskBg(score: number): string {
  if (score <= 25) return "rgba(77,87,65,0.08)";
  if (score <= 50) return "rgba(208,176,96,0.08)";
  return "rgba(196,87,74,0.06)";
}
  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="show">
      ...
      <motion.div variants={fadeUp} className="mb-7">
        <div className="flex items-end justify-between gap-6">
          ...
          <Link href="/enterprise/sow/intake">
            <button className="flex items-center gap-1.5 rounded-xl text-white text-xs font-medium px-4 py-2 shrink-0 transition-all hover:-translate-y-0.5"
              style={{ background: "linear-gradient(135deg, #A67763, #886151)", boxShadow: "0 2px 8px rgba(0,0,0,0.12)" }}>
      ...
      <motion.div variants={fadeUp} className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-7">
        {[
          { label: "Total SOWs", value: totalSOWs, icon: FileText },
          { label: "Pending Action", value: pendingCount, icon: AlertTriangle },
          { label: "Approved", value: approvedCount, icon: Shield },
          { label: "Avg Risk", value: avgRisk, icon: AlertTriangle },
          { label: "Total Budget", value: `$${(totalBudget / 1000).toFixed(0)}K`, icon: DollarSign },
        ].map((kpi) => (
      ...
      {mockSOWs.length === 0 ? (
        <motion.div variants={fadeUp} className="card-parchment">
          <div className="flex flex-col items-center justify-center py-20 text-center px-6">
            <div className="flex items-center justify-center mb-4 w-[52px] h-[52px] rounded-2xl bg-white/60 border border-white/30">
            ...
            <Link href="/enterprise/sow/intake">
              <button className="flex items-center gap-1.5 rounded-xl text-white text-xs font-medium px-4 py-2"
                style={{ background: "linear-gradient(135deg, #A67763, #886151)", boxShadow: "0 2px 8px rgba(0,0,0,0.12)" }}>
                <Plus className="w-3 h-3" /> New SOW
              </button>
            </Link>
        ...
        <motion.div variants={fadeUp} className="card-parchment mb-5">
          ...
          {/* Search + Filters */}
          <div className="flex items-center justify-between gap-3 px-6 py-3" style={{ borderBottom: "1px solid rgba(0,0,0,0.04)" }}>
            {/* Search */}
            <div
            ...
            >
              ...
              {search ? (
                <button onClick={() => setSearch("")} className="shrink-0 p-0.5 rounded text-gray-400"><X className="w-3 h-3" /></button>
              ) : !searchFocused ? (
                <kbd className="font-mono whitespace-nowrap shrink-0 text-[9px] text-gray-400 bg-black/[0.04] border border-black/[0.06] px-1.5 py-px rounded">⌘F</kbd>
              ) : null}
            </div>
            ...
            <div className="flex items-center gap-2.5">
              ...
              {activeFilterCount > 0 && (
                <button onClick={clearAllFilters} className="flex items-center gap-1.5 text-[11px] font-medium text-brown-500 px-2.5 py-1 rounded-lg hover:bg-white/50 transition-all">
          ...
          <div className="overflow-x-auto">
                    ...
                    return (
                      <th key={col.field} onClick={() => handleSort(col.field)}
                        ...
                        style={{
                          ...
                          color: active ? "var(--ink-mid)" : "var(--ink-faint)", background: "rgba(0,0,0,0.01)",
                        }}>
                  ...
                  return (
                    <tr key={sow.id} onClick={() => router.push(`/enterprise/sow/${sow.id}`)}
                      ...
                      <td style={{ padding: "13px 16px" }}>
                        <div className="text-[13px] font-medium text-gray-800 truncate max-w-[260px]">{sow.title}</div>
                      ...
                      <td style={{ padding: "13px 16px" }}>
                        <Pill bg={sow.intakeMode === "ai_generated" ? "rgba(91,155,162,0.08)" : "rgba(0,0,0,0.04)"} color={sow.intakeMode === "ai_generated" ? "#3A6368" : "#706B66"}>
            ...
            {filtered.length === 0 && (
              <div className="flex flex-col items-center justify-center py-16 text-center px-6">
                <div className="flex items-center justify-center mb-4 w-12 h-12 rounded-2xl bg-white/60 border border-white/30">
                ...
                <button onClick={clearAllFilters}
                  className="flex items-center gap-1.5 rounded-xl text-xs font-medium text-brown-500 px-3.5 py-1.5 border border-brown-200 hover:bg-white/50 transition-all">
          ...
          {filtered.length > pageSize && (
            <div className="flex items-center justify-between px-6 py-3" style={{ borderTop: "1px solid rgba(0,0,0,0.04)" }}>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage <= 1}
                    className="flex items-center justify-center w-7 h-7 rounded-lg text-gray-500 bg-white/50 border border-white/30 hover:bg-white/70 transition-all disabled:opacity-30 disabled:cursor-not-allowed">
