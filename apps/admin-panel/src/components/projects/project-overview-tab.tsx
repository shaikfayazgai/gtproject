...
export function ProjectOverviewTab({ projectId, project }: ProjectOverviewTabProps) {
  ...
  const { data: activity, isLoading: activityLoading } = useQuery<APGAction[]>({
    queryKey: ['admin-project-apg-overview', projectId],
    queryFn: async () => {
  ...
  // Budget data (would come from project detail in real implementation)
  const budgetTotal = 200000
  const budgetReleased = 78000
  const budgetPending = 22000
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Recent APG Activity</CardTitle>
        </CardHeader>
        <CardContent>
          {activityLoading ? (
            <div className="flex justify-center py-6">
